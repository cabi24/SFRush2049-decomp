...
5. Data-reference symbolization (D3): for `lui`+`%lo`-offset pairs whose
   effective address is in the committed game symbol table, rewrite to
   `%hi(name)`/`%lo(name)`. All other data refs stay numeric — numeric
   absolute addresses are score-correct against raw-word target objects.
