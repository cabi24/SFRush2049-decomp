...
def _opt_of(flagset):
    """The -O token of a segment flagset (e.g. '-O1'), or None."""
    if not flagset:
        return None
    m = re.search(r"-O[0-9s]", flagset)
    return m.group(0) if m else None


def write_opt_overrides(mapping=None):
    """Emit committed per-ROM-TU CFLAGS overrides for converted segments whose
    match was verified at a non-default opt level.

    The Makefile compiles every src/rom/*.c with the global OPT_FLAGS (-O2). A
    function matched at -O1 (the libultra/os family) would diverge and fail the
    promotion SHA-1 gate (osCreateMesgQueue, 0x7600). This fragment overrides
    CFLAGS for those TUs; it is committed so it reaches the builder via git —
    promotion only rsyncs the TU, not the Makefile."""
    if mapping is None:
        mapping = derive()
    lines = [
    ...
    for seg in mapping["segments"]:
        if not seg["converted"]:
            continue
        opt = _opt_of(seg["flagset"])
        if opt in (None, "-O2"):
            continue
        obj = f"$(BUILD_DIR)/src/{seg['rom_tu']}.o"
        lines.append(f"{obj}: CFLAGS := $(subst -O2,{opt},$(CFLAGS))")
    lines.append("endif")
    OPT_OVERRIDES_MK.parent.mkdir(parents=True, exist_ok=True)
    OPT_OVERRIDES_MK.write_text("\n".join(lines) + "\n")
...
def _do_convert(mapping, seg):
    ...
    write_opt_overrides()  # re-derive: seg is now converted on disk
