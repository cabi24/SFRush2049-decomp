...
def _cmd_convert(args):
    # T005: BLOCKED. `convert` must run splat re-split (`make extract`) to emit
    # the per-function nonmatchings asm and regenerate the linker script (D2/D3;
    # rule 5 forbids hand-editing those). splat 0.37.1 cannot parse the current
    # symbol_addrs.us.txt (multi-colon comments, type:data, duplicate
    # addresses/names), so re-split is non-operational and cannot reproduce the
    # checked-in asm. See specs/004-promotion-splicing/{tasks,quickstart}.md.
    sys.exit(
        "layout convert is BLOCKED: splat re-split (make extract) does not run "
        "against the current symbol_addrs.us.txt. See "
        "specs/004-promotion-splicing/quickstart.md §2 for evidence and the "
        "reviewer decision required before conversion can proceed."
    )
