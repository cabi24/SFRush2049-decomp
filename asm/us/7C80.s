.include "macro.inc"

.set noat
.set noreorder
.set gp=64

.section .text, "ax"

/* Generated by spimdisasm 1.42.2 */

nonmatching osStartThread, 0x150

glabel osStartThread
    /* 7C80 80007080 27BDFFD8 */  addiu      $sp, $sp, -0x28
    /* 7C84 80007084 AFBF001C */  sw         $ra, 0x1C($sp)
    /* 7C88 80007088 AFA40028 */  sw         $a0, 0x28($sp)
    /* 7C8C 8000708C AFB10018 */  sw         $s1, 0x18($sp)
    /* 7C90 80007090 0C00312C */  jal        __osDisableInt
    /* 7C94 80007094 AFB00014 */   sw        $s0, 0x14($sp)
    /* 7C98 80007098 8FAE0028 */  lw         $t6, 0x28($sp)
    /* 7C9C 8000709C 24010001 */  addiu      $at, $zero, 0x1
    /* 7CA0 800070A0 00408025 */  or         $s0, $v0, $zero
    /* 7CA4 800070A4 95D10010 */  lhu        $s1, 0x10($t6)
    /* 7CA8 800070A8 1221000C */  beq        $s1, $at, .L800070DC
    /* 7CAC 800070AC 24010008 */   addiu     $at, $zero, 0x8
    /* 7CB0 800070B0 1621002A */  bne        $s1, $at, .L8000715C
    /* 7CB4 800070B4 00000000 */   nop
    /* 7CB8 800070B8 8FB80028 */  lw         $t8, 0x28($sp)
    /* 7CBC 800070BC 240F0002 */  addiu      $t7, $zero, 0x2
    /* 7CC0 800070C0 3C048003 */  lui        $a0, %hi(__osActiveQueue)
    /* 7CC4 800070C4 A70F0010 */  sh         $t7, 0x10($t8)
    /* 7CC8 800070C8 8FA50028 */  lw         $a1, 0x28($sp)
    /* 7CCC 800070CC 0C003431 */  jal        __osEnqueueThread
    /* 7CD0 800070D0 2484C3D8 */   addiu     $a0, $a0, %lo(__osActiveQueue)
    /* 7CD4 800070D4 10000021 */  b          .L8000715C
    /* 7CD8 800070D8 00000000 */   nop
  .L800070DC:
    /* 7CDC 800070DC 8FB90028 */  lw         $t9, 0x28($sp)
    /* 7CE0 800070E0 8F280008 */  lw         $t0, 0x8($t9)
    /* 7CE4 800070E4 11000005 */  beqz       $t0, .L800070FC
    /* 7CE8 800070E8 00000000 */   nop
    /* 7CEC 800070EC 3C098003 */  lui        $t1, %hi(__osActiveQueue)
    /* 7CF0 800070F0 2529C3D8 */  addiu      $t1, $t1, %lo(__osActiveQueue)
    /* 7CF4 800070F4 1509000A */  bne        $t0, $t1, .L80007120
    /* 7CF8 800070F8 00000000 */   nop
  .L800070FC:
    /* 7CFC 800070FC 8FAB0028 */  lw         $t3, 0x28($sp)
    /* 7D00 80007100 240A0002 */  addiu      $t2, $zero, 0x2
    /* 7D04 80007104 3C048003 */  lui        $a0, %hi(__osActiveQueue)
    /* 7D08 80007108 A56A0010 */  sh         $t2, 0x10($t3)
    /* 7D0C 8000710C 8FA50028 */  lw         $a1, 0x28($sp)
    /* 7D10 80007110 0C003431 */  jal        __osEnqueueThread
    /* 7D14 80007114 2484C3D8 */   addiu     $a0, $a0, %lo(__osActiveQueue)
    /* 7D18 80007118 10000010 */  b          .L8000715C
    /* 7D1C 8000711C 00000000 */   nop
  .L80007120:
    /* 7D20 80007120 8FAD0028 */  lw         $t5, 0x28($sp)
    /* 7D24 80007124 240C0008 */  addiu      $t4, $zero, 0x8
    /* 7D28 80007128 A5AC0010 */  sh         $t4, 0x10($t5)
    /* 7D2C 8000712C 8FAE0028 */  lw         $t6, 0x28($sp)
    /* 7D30 80007130 8DC40008 */  lw         $a0, 0x8($t6)
    /* 7D34 80007134 0C003431 */  jal        __osEnqueueThread
    /* 7D38 80007138 01C02825 */   or        $a1, $t6, $zero
    /* 7D3C 8000713C 8FAF0028 */  lw         $t7, 0x28($sp)
    /* 7D40 80007140 0C003443 */  jal        __osPopThread
    /* 7D44 80007144 8DE40008 */   lw        $a0, 0x8($t7)
    /* 7D48 80007148 00408825 */  or         $s1, $v0, $zero
    /* 7D4C 8000714C 3C048003 */  lui        $a0, %hi(__osActiveQueue)
    /* 7D50 80007150 2484C3D8 */  addiu      $a0, $a0, %lo(__osActiveQueue)
    /* 7D54 80007154 0C003431 */  jal        __osEnqueueThread
    /* 7D58 80007158 02202825 */   or        $a1, $s1, $zero
  .L8000715C:
    /* 7D5C 8000715C 3C188003 */  lui        $t8, %hi(__osRunningThread)
    /* 7D60 80007160 8F18C3E0 */  lw         $t8, %lo(__osRunningThread)($t8)
    /* 7D64 80007164 17000005 */  bnez       $t8, .L8000717C
    /* 7D68 80007168 00000000 */   nop
    /* 7D6C 8000716C 0C003447 */  jal        __osDispatchThread
    /* 7D70 80007170 00000000 */   nop
    /* 7D74 80007174 1000000F */  b          .L800071B4
    /* 7D78 80007178 00000000 */   nop
  .L8000717C:
    /* 7D7C 8000717C 3C198003 */  lui        $t9, %hi(__osRunningThread)
    /* 7D80 80007180 3C098003 */  lui        $t1, %hi(__osActiveQueue)
    /* 7D84 80007184 8D29C3D8 */  lw         $t1, %lo(__osActiveQueue)($t1)
    /* 7D88 80007188 8F39C3E0 */  lw         $t9, %lo(__osRunningThread)($t9)
    /* 7D8C 8000718C 8D2A0004 */  lw         $t2, 0x4($t1)
    /* 7D90 80007190 8F280004 */  lw         $t0, 0x4($t9)
    /* 7D94 80007194 010A082A */  slt        $at, $t0, $t2
    /* 7D98 80007198 10200006 */  beqz       $at, .L800071B4
    /* 7D9C 8000719C 00000000 */   nop
    /* 7DA0 800071A0 240B0002 */  addiu      $t3, $zero, 0x2
    /* 7DA4 800071A4 3C048003 */  lui        $a0, %hi(__osActiveQueue)
    /* 7DA8 800071A8 A72B0010 */  sh         $t3, 0x10($t9)
    /* 7DAC 800071AC 0C0033F1 */  jal        __osCleanupThread
    /* 7DB0 800071B0 2484C3D8 */   addiu     $a0, $a0, %lo(__osActiveQueue)
  .L800071B4:
    /* 7DB4 800071B4 0C003148 */  jal        __osRestoreInt
    /* 7DB8 800071B8 02002025 */   or        $a0, $s0, $zero
    /* 7DBC 800071BC 8FBF001C */  lw         $ra, 0x1C($sp)
    /* 7DC0 800071C0 8FB00014 */  lw         $s0, 0x14($sp)
    /* 7DC4 800071C4 8FB10018 */  lw         $s1, 0x18($sp)
    /* 7DC8 800071C8 03E00008 */  jr         $ra
    /* 7DCC 800071CC 27BD0028 */   addiu     $sp, $sp, 0x28
endlabel osStartThread

/* Handwritten function */
nonmatching osSetGlobalIntMask, 0xA0

glabel osSetGlobalIntMask
    /* 7DD0 800071D0 400C6000 */  mfc0       $t4, $12 /* handwritten instruction */
    /* 7DD4 800071D4 3182FF01 */  andi       $v0, $t4, 0xFF01
    /* 7DD8 800071D8 3C088003 */  lui        $t0, %hi(__osGlobalIntMask)
    /* 7DDC 800071DC 2508C370 */  addiu      $t0, $t0, %lo(__osGlobalIntMask)
    /* 7DE0 800071E0 8D0B0000 */  lw         $t3, 0x0($t0)
    /* 7DE4 800071E4 2401FFFF */  addiu      $at, $zero, -0x1
    /* 7DE8 800071E8 01614026 */  xor        $t0, $t3, $at
    /* 7DEC 800071EC 3108FF00 */  andi       $t0, $t0, 0xFF00
    /* 7DF0 800071F0 00481025 */  or         $v0, $v0, $t0
    /* 7DF4 800071F4 3C0AA430 */  lui        $t2, %hi(MI_INTR_MASK_REG)
    /* 7DF8 800071F8 8D4A000C */  lw         $t2, %lo(MI_INTR_MASK_REG)($t2)
    /* 7DFC 800071FC 11400005 */  beqz       $t2, .L80007214
    /* 7E00 80007200 000B4C02 */   srl       $t1, $t3, 16
    /* 7E04 80007204 2401FFFF */  addiu      $at, $zero, -0x1
    /* 7E08 80007208 01214826 */  xor        $t1, $t1, $at
    /* 7E0C 8000720C 3129003F */  andi       $t1, $t1, 0x3F
    /* 7E10 80007210 01495025 */  or         $t2, $t2, $t1
  .L80007214:
    /* 7E14 80007214 000A5400 */  sll        $t2, $t2, 16
    /* 7E18 80007218 004A1025 */  or         $v0, $v0, $t2
    /* 7E1C 8000721C 3C01003F */  lui        $at, (0x3F0000 >> 16)
    /* 7E20 80007220 00814024 */  and        $t0, $a0, $at
    /* 7E24 80007224 010B4024 */  and        $t0, $t0, $t3
    /* 7E28 80007228 000843C2 */  srl        $t0, $t0, 15
    /* 7E2C 8000722C 3C0A8003 */  lui        $t2, %hi(__osRcpImTable)
    /* 7E30 80007230 01485021 */  addu       $t2, $t2, $t0
    /* 7E34 80007234 954AD6D0 */  lhu        $t2, %lo(__osRcpImTable)($t2)
    /* 7E38 80007238 3C01A430 */  lui        $at, %hi(MI_INTR_MASK_REG)
    /* 7E3C 8000723C AC2A000C */  sw         $t2, %lo(MI_INTR_MASK_REG)($at)
    /* 7E40 80007240 3088FF01 */  andi       $t0, $a0, 0xFF01
    /* 7E44 80007244 3169FF00 */  andi       $t1, $t3, 0xFF00
    /* 7E48 80007248 01094024 */  and        $t0, $t0, $t1
    /* 7E4C 8000724C 3C01FFFF */  lui        $at, (0xFFFF00FF >> 16)
    /* 7E50 80007250 342100FF */  ori        $at, $at, (0xFFFF00FF & 0xFFFF)
    /* 7E54 80007254 01816024 */  and        $t4, $t4, $at
    /* 7E58 80007258 01886025 */  or         $t4, $t4, $t0
    /* 7E5C 8000725C 408C6000 */  mtc0       $t4, $12 /* handwritten instruction */
    /* 7E60 80007260 00000000 */  nop
    /* 7E64 80007264 00000000 */  nop
    /* 7E68 80007268 03E00008 */  jr         $ra
    /* 7E6C 8000726C 00000000 */   nop
endlabel osSetGlobalIntMask

nonmatching osRecvMesg, 0x138

glabel osRecvMesg
    /* 7E70 80007270 27BDFFD8 */  addiu      $sp, $sp, -0x28
    /* 7E74 80007274 AFBF001C */  sw         $ra, 0x1C($sp)
    /* 7E78 80007278 AFA40028 */  sw         $a0, 0x28($sp)
    /* 7E7C 8000727C AFA5002C */  sw         $a1, 0x2C($sp)
    /* 7E80 80007280 AFA60030 */  sw         $a2, 0x30($sp)
    /* 7E84 80007284 AFB10018 */  sw         $s1, 0x18($sp)
    /* 7E88 80007288 0C00312C */  jal        __osDisableInt
    /* 7E8C 8000728C AFB00014 */   sw        $s0, 0x14($sp)
    /* 7E90 80007290 8FAE0028 */  lw         $t6, 0x28($sp)
    /* 7E94 80007294 00408025 */  or         $s0, $v0, $zero
    /* 7E98 80007298 8DCF0008 */  lw         $t7, 0x8($t6)
    /* 7E9C 8000729C 15E00012 */  bnez       $t7, .L800072E8
    /* 7EA0 800072A0 00000000 */   nop
  .L800072A4:
    /* 7EA4 800072A4 8FB80030 */  lw         $t8, 0x30($sp)
    /* 7EA8 800072A8 17000005 */  bnez       $t8, .L800072C0
    /* 7EAC 800072AC 00000000 */   nop
    /* 7EB0 800072B0 0C003148 */  jal        __osRestoreInt
    /* 7EB4 800072B4 02002025 */   or        $a0, $s0, $zero
    /* 7EB8 800072B8 10000036 */  b          .L80007394
    /* 7EBC 800072BC 2402FFFF */   addiu     $v0, $zero, -0x1
  .L800072C0:
    /* 7EC0 800072C0 3C088003 */  lui        $t0, %hi(__osRunningThread)
    /* 7EC4 800072C4 8D08C3E0 */  lw         $t0, %lo(__osRunningThread)($t0)
    /* 7EC8 800072C8 24190008 */  addiu      $t9, $zero, 0x8
    /* 7ECC 800072CC A5190010 */  sh         $t9, 0x10($t0)
    /* 7ED0 800072D0 0C0033F1 */  jal        __osCleanupThread
    /* 7ED4 800072D4 8FA40028 */   lw        $a0, 0x28($sp)
    /* 7ED8 800072D8 8FA90028 */  lw         $t1, 0x28($sp)
    /* 7EDC 800072DC 8D2A0008 */  lw         $t2, 0x8($t1)
    /* 7EE0 800072E0 1140FFF0 */  beqz       $t2, .L800072A4
    /* 7EE4 800072E4 00000000 */   nop
  .L800072E8:
    /* 7EE8 800072E8 8FAB002C */  lw         $t3, 0x2C($sp)
    /* 7EEC 800072EC 11600008 */  beqz       $t3, .L80007310
    /* 7EF0 800072F0 00000000 */   nop
    /* 7EF4 800072F4 8FAC0028 */  lw         $t4, 0x28($sp)
    /* 7EF8 800072F8 8D8E000C */  lw         $t6, 0xC($t4)
    /* 7EFC 800072FC 8D8D0014 */  lw         $t5, 0x14($t4)
    /* 7F00 80007300 000E7880 */  sll        $t7, $t6, 2
    /* 7F04 80007304 01AFC021 */  addu       $t8, $t5, $t7
    /* 7F08 80007308 8F190000 */  lw         $t9, 0x0($t8)
    /* 7F0C 8000730C AD790000 */  sw         $t9, 0x0($t3)
  .L80007310:
    /* 7F10 80007310 8FA80028 */  lw         $t0, 0x28($sp)
    /* 7F14 80007314 8D09000C */  lw         $t1, 0xC($t0)
    /* 7F18 80007318 8D0C0010 */  lw         $t4, 0x10($t0)
    /* 7F1C 8000731C 252A0001 */  addiu      $t2, $t1, 0x1
    /* 7F20 80007320 014C001A */  div        $zero, $t2, $t4
    /* 7F24 80007324 00007010 */  mfhi       $t6
    /* 7F28 80007328 AD0E000C */  sw         $t6, 0xC($t0)
    /* 7F2C 8000732C 8FAD0028 */  lw         $t5, 0x28($sp)
    /* 7F30 80007330 15800002 */  bnez       $t4, .L8000733C
    /* 7F34 80007334 00000000 */   nop
    /* 7F38 80007338 0007000D */  break      7
  .L8000733C:
    /* 7F3C 8000733C 2401FFFF */  addiu      $at, $zero, -0x1
    /* 7F40 80007340 15810004 */  bne        $t4, $at, .L80007354
    /* 7F44 80007344 3C018000 */   lui       $at, (0x80000000 >> 16)
    /* 7F48 80007348 15410002 */  bne        $t2, $at, .L80007354
    /* 7F4C 8000734C 00000000 */   nop
    /* 7F50 80007350 0006000D */  break      6
  .L80007354:
    /* 7F54 80007354 8DAF0008 */  lw         $t7, 0x8($t5)
    /* 7F58 80007358 25F8FFFF */  addiu      $t8, $t7, -0x1
    /* 7F5C 8000735C ADB80008 */  sw         $t8, 0x8($t5)
    /* 7F60 80007360 8FB90028 */  lw         $t9, 0x28($sp)
    /* 7F64 80007364 8F2B0004 */  lw         $t3, 0x4($t9)
    /* 7F68 80007368 8D690000 */  lw         $t1, 0x0($t3)
    /* 7F6C 8000736C 11200006 */  beqz       $t1, .L80007388
    /* 7F70 80007370 00000000 */   nop
    /* 7F74 80007374 0C003443 */  jal        __osPopThread
    /* 7F78 80007378 27240004 */   addiu     $a0, $t9, 0x4
    /* 7F7C 8000737C 00408825 */  or         $s1, $v0, $zero
    /* 7F80 80007380 0C001C20 */  jal        osStartThread
    /* 7F84 80007384 02202025 */   or        $a0, $s1, $zero
  .L80007388:
    /* 7F88 80007388 0C003148 */  jal        __osRestoreInt
    /* 7F8C 8000738C 02002025 */   or        $a0, $s0, $zero
    /* 7F90 80007390 00001025 */  or         $v0, $zero, $zero
  .L80007394:
    /* 7F94 80007394 8FBF001C */  lw         $ra, 0x1C($sp)
    /* 7F98 80007398 8FB00014 */  lw         $s0, 0x14($sp)
    /* 7F9C 8000739C 8FB10018 */  lw         $s1, 0x18($sp)
    /* 7FA0 800073A0 03E00008 */  jr         $ra
    /* 7FA4 800073A4 27BD0028 */   addiu     $sp, $sp, 0x28
endlabel osRecvMesg
    /* 7FA8 800073A8 00000000 */  nop
    /* 7FAC 800073AC 00000000 */  nop
